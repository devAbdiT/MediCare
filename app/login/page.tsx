      ...
      <div className="hidden lg:flex lg:w-[45%] bg-[#1E4A8A] dark:bg-[#111C3A] relative overflow-hidden flex-col justify-between p-16 rounded-[3.5rem] shadow-2xl transition-all duration-700">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-16">
            ...
            <div>
              <span className="text-3xl font-black tracking-tighter text-white block leading-none">MediCare</span>
              <span className="text-xs font-black text-[#4A8AC8] uppercase tracking-[0.4em] mt-2 block">Enterprise OS</span>
            </div>
          </div>
          ...
          <div className="space-y-8 max-w-sm">
             <div className="bg-white/10 backdrop-blur-xl border border-white/10 p-6 rounded-[2rem] flex items-center gap-4 hover:bg-white/20 transition-all">
                ...
                <p className="text-sm font-bold text-white leading-tight">Quantum-grade data encryption active.</p>
             </div>
             <div className="bg-white/10 backdrop-blur-xl border border-white/10 p-6 rounded-[2rem] flex items-center gap-4 hover:bg-white/20 transition-all">
                <div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center text-white">
                  <Activity size={24} />
                </div>
                <p className="text-sm font-bold text-white leading-tight">Real-time clinical synchronization.</p>
             </div>
          </div>
        </div>
        ...
        <div className="relative z-10 flex items-center gap-6 pt-12 border-t border-white/10">
           <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-white uppercase tracking-widest">SLA 100%</span>
           </div>
           <div className="flex items-center gap-2">
              <Lock size={14} className="text-[#8A9CBA]" />
              <span className="text-[10px] font-black text-white uppercase tracking-widest">AES-256</span>
           </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center relative overflow-y-auto custom-scrollbar">
        <div className="w-full max-w-lg space-y-10 p-8 lg:p-0">
          <div className="text-center lg:text-left space-y-4">
            <h2 className="text-5xl font-black text-[#1A2A4A] dark:text-[#E8EEF8] tracking-tighter">Authorization</h2>
            <p className="text-[#5A6E8A] dark:text-[#8A9CBA] text-lg font-medium">Identify yourself to enter the Command Center.</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-8">
            <div className="space-y-3">
              <label className="text-[10px] font-black text-[#5A6E8A] dark:text-[#8A9CBA] uppercase tracking-[0.3em] ml-2">Clinical Identifier (Email)</label>
              <div className="relative group">
                <User className="absolute left-6 top-1/2 -translate-y-1/2 text-[#5A6E8A] group-focus-within:text-[#1E4A8A] transition-colors" size={20} />
                <input
                  type="email"
                  value={email}
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black text-[#5A6E8A] dark:text-[#8A9CBA] uppercase tracking-[0.3em] ml-2">Secure Passkey</label>
              <div className="relative group">
                <Lock className="absolute left-6 top-1/2 -translate-y-1/2 text-[#5A6E8A] group-focus-within:text-[#1E4A8A] transition-colors" size={20} />
                <input
                  type="password"
                  value={password}
            ...
            <div className="flex items-center justify-between px-2">
              <label className="flex items-center gap-3 cursor-pointer group">
                <div className="relative flex items-center">
                   ...
                   <span className="ml-3 text-sm font-black text-[#5A6E8A] dark:text-[#8A9CBA] uppercase tracking-widest">Keep Sync Active</span>
                </div>
              </label>
              <a href="#" className="text-xs font-black text-[#1E4A8A] dark:text-[#4A8AC8] uppercase tracking-widest hover:underline underline-offset-4">Reset Credentials</a>
            </div>
          ...
          <div className="p-8 bg-blue-50/50 dark:bg-blue-900/10 rounded-[2.5rem] border border-[#D0DCE8] dark:border-[#1A2A4A]">
            <h4 className="text-[10px] font-black text-[#1E4A8A] dark:text-[#4A8AC8] uppercase tracking-[0.5em] mb-6 text-center">Node Access Manifest</h4>
            <div className="grid grid-cols-2 gap-6 text-[10px] font-black text-[#5A6E8A] dark:text-[#8A9CBA] uppercase tracking-widest">
              ...
              <DemoItem role="Staff" email="reception@hospital.com" />
              <DemoItem role="Patient" email="patient@hospital.com" />
            </div>
            <p className="text-[9px] text-[#1E4A8A]/50 dark:text-[#4A8AC8]/30 mt-6 text-center font-black uppercase tracking-[0.3em]">Access Key for All Nodes: password123</p>
          </div>
        </div>
        ...
        <div className="absolute bottom-12 right-12 flex items-center gap-3 opacity-20">
           ...
           <span className="text-[10px] font-black text-[#1A2A4A] dark:text-[#E8EEF8] uppercase tracking-[0.5em]">Global Clinical OS v2.4.1</span>
        </div>
      </div>
    </div>
  );
...
function DemoItem({ role, email }: { role: string; email: string }) {
  return (
    <div className="flex flex-col gap-1">
      ...
      <span className="opacity-60 lowercase font-medium tracking-tight text-[11px]">{email}</span>
    </div>
  );
}
