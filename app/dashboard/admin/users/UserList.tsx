    ...
    <div className="space-y-8">
      ...
      <div className="print-only mb-10 pb-6 border-b-2 border-slate-900">
        <div className="flex justify-between items-start">
          ...
          <div className="text-right">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-900 mt-1">
              Printed By: {adminName || "System Administrator"}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col xl:flex-row justify-between gap-6">
        ...
        <div className="relative group min-w-[300px]">
          <Search
            className="absolute left-4 top-1/2 -translate-y-1/2 text-[#5A6E8A] group-focus-within:text-[#1E4A8A] transition-colors"
            size={18}
          />
          <input
            type="text"
        ...
        <div className="flex gap-2 no-print">
          <Dialog>
            <DialogTrigger asChild>
              <Button
                variant="outline"
                className="flex items-center gap-2 px-6 py-4 bg-white dark:bg-[#111C3A] text-[#1E4A8A] dark:text-[#4A8AC8] rounded-2xl font-black text-[10px] uppercase tracking-widest border border-[#D0DCE8] dark:border-[#1A2A4A] shadow-sm hover:bg-[#F0F4F8] dark:hover:bg-[#0A122A] transition-all"
              >
                <Printer size={16} />
                Print List Manifest
              </Button>
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-900 border-none rounded-[2rem]">
              ...
              <PrintUserList
                ...
                title={
                  activeTab === "ALL"
                    ? "Global Personnel Manifest"
                    : `${activeTab.charAt(0) + activeTab.slice(1).toLowerCase()}s Directory`
                }
              />
              ...
              {filteredUsers.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-8 py-20 text-center">
                    <p className="text-[#5A6E8A] dark:text-[#8A9CBA] font-bold italic">
                      No matching records found in the current node.
                    </p>
                  </td>
                </tr>
              ) : (
                filteredUsers.map((user) => (
                  <tr
                    key={user.id}
                    className="hover:bg-[#F0F4F8]/50 dark:hover:bg-[#0A122A]/50 transition-colors group"
                  >
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black ${roleColors[user.role as keyof typeof roleColors].bg} ${roleColors[user.role as keyof typeof roleColors].text}`}
                        >
                          {user.name[0]}
                        </div>
                        <div>
                          <p className="font-bold text-[#1A2A4A] dark:text-[#E8EEF8]">
                            {user.name}
                          </p>
                          <p className="text-[10px] text-[#5A6E8A] dark:text-[#8A9CBA] font-bold">
                            {user.email}
                          </p>
                        </div>
                      </div>
                    ...
                    <td className="px-8 py-6">
                      <div className="space-y-1">
                        <div
                          className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${roleColors[user.role as keyof typeof roleColors].bg} ${roleColors[user.role as keyof typeof roleColors].text}`}
                        >
                          <Shield size={10} />
                          {user.role}
                        </div>
                        {user.doctor?.specialization && (
                          <p className="text-[10px] font-bold text-[#5A6E8A] dark:text-[#8A9CBA] mt-1">
                            {user.doctor.specialization}
                          </p>
                        )}
                        {user.patient?.bloodType && (
                          <p className="text-[10px] font-bold text-[#2D8A6E] dark:text-[#4AA88A] mt-1">
                            Blood: {user.patient.bloodType.replace("_", "+")}
                          </p>
                        )}
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      {user.role === "PATIENT" ? (
                        ...
                        <span className="text-[#8A9CBA] text-xs font-bold">
                          —
                        </span>
                      )}
                    </td>
                        ...
                        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-900 border-none rounded-[2rem]">
                          ...
                          {loadingPrint ? (
                            <div className="flex flex-col items-center justify-center py-20 gap-4">
                              <Loader2
                                className="animate-spin text-[#1E4A8A]"
                                size={40}
                              />
                              <p className="text-sm font-black uppercase tracking-widest text-[#5A6E8A]">
...
const roleColors = {
  ...
  DOCTOR: {
    ...
    text: "text-[#1E4A8A] dark:text-[#4A8AC8]",
  },
  RECEPTIONIST: {
  ...
  PATIENT: {
    ...
    text: "text-[#2D8A6E] dark:text-[#4AA88A]",
  },
};
