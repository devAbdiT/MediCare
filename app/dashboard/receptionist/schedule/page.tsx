...
export default function ReceptionistSchedulePage() {
  ...
  const statusColors = {
    ...
    COMPLETED: "bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-900",
    ...
    <DashboardLayout role="receptionist">
      <div className="max-w-6xl mx-auto space-y-8 pb-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-4xl font-black text-slate-900 dark:text-slate-100 tracking-tight">{showAll ? "Master Schedule" : "Today's Schedule"}</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-2 text-lg font-medium">{showAll ? "All upcoming appointments" : format(new Date(), "EEEE, MMMM do, yyyy")}</p>
          </div>
          <div className="flex items-center gap-4">
            ...
            <button 
              onClick={handlePrint}
              className="px-6 py-3 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-2xl font-bold flex items-center gap-2 transition-colors print:hidden"
            >
              <Printer size={20} />
              Print
            </button>
                    ...
                    <tr key={app.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-colors group">
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-bold text-slate-900 dark:text-slate-100">{app.patient?.user?.name || "Unknown"}</p>
                            <p className="text-xs text-slate-500 dark:text-slate-400">{app.patient?.user?.phone || "No phone"}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-6">
